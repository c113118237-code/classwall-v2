...
-- 3b. 不喜歡去重表：一個 anon_id 對一題只能按不喜歡一次（DB 端強制）
...
-- ⚠️ question_likes 和 question_dislikes 故意「不給任何 policy」→ anon 完全摸不到，
...
-- 6b. RPC：原子地寫去重 row + bump dislikes
-- 同上，SECURITY DEFINER 且 set search_path 鎖死
...
-- 7. seed：一筆示範資料
insert into public.questions (content, likes, dislikes)
values ('歡迎來到 ClassWall！按下「我也想問 +1」試試看 🎉', 0, 0)
on conflict do nothing;
